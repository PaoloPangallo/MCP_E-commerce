  ...
  const run = useCallback((query: string, image?: string) => {
    ...
    const nextSource = streamAgent(query, image, (event: AgentEvent) => {
      ...
      if (event.type === "answer_chunk") {
        ...
        const now = Date.now()
        // Throttle React state updates to ~20 FPS (50ms) to prevent UI freezing
        // extremely common issue when dealing with rapid LLM stream chunks and heavy React layouts.
