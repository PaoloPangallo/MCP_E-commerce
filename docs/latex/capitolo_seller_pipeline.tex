...
\subsection{Approccio Principale: Embedding Semantici con Anchor Vectors}
\label{subsec:sentiment-embedding}

La strategia principale utilizza il modello
\textbf{all-MiniLM-L6-v2} (SentenceTransformers) per calcolare
embedding densi a 384 dimensioni per ogni testo di feedback. Il modello
è condiviso nell'applicazione tramite un singleton thread-safe
...
Il punteggio di sentiment non è calcolato con un classificatore
addestrato, ma tramite il confronto geometrico nello spazio degli
embedding con due insiemi di \textbf{anchor vector}:

\begin{itemize}
    \item \textbf{Anchor positivi} — frasi prototipiche di feedback
        eccellente, in inglese e italiano (es.\ \textit{"excellent seller
        fast shipping"}, \textit{"ottimo venditore spedizione veloce"}).
    \item \textbf{Anchor negativi} — frasi prototipiche di feedback
        critico (es.\ \textit{"terrible seller never again"},
        \textit{"truffa non comprate"}).
\end{itemize}

Gli anchor vector sono calcolati una sola volta e memorizzati con
\texttt{@lru\_cache(maxsize=1)}, eliminando il costo di ricalcolo
a ogni invocazione della pipeline.

Per ogni testo di feedback, il punteggio grezzo è:

\[
    \text{raw}_i = \max_j(\text{emb}_i \cdot \text{pos}_j)
                 - \max_k(\text{emb}_i \cdot \text{neg}_k)
\]

dove \(\text{emb}_i\) è l'embedding normalizzato del testo $i$,
\(\text{pos}_j\) e \(\text{neg}_k\) sono rispettivamente gli anchor
positivi e negativi normalizzati. Il prodotto scalare tra vettori
normalizzati equivale alla cosine similarity.

Il punteggio grezzo viene poi compresso nell'intervallo $[0, 1]$
tramite funzione sigmoide con guadagno 3, che amplifica la separazione
attorno allo zero:

\[
    s_i = \frac{1}{1 + e^{-3 \cdot \text{raw}_i}}
\]

Il sentiment score complessivo del venditore è la media aritmetica
degli $s_i$ calcolati sui feedback con testo valido.

\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.78\linewidth}{\centering\vspace{2cm}
        \textit{[Diagramma: Testo feedback → all-MiniLM-L6-v2 →
        Embedding [384d] → Cosine sim vs anchor+/anchor- →
        raw score → Sigmoid(3x) → $s_i \in [0,1]$]}\\
        \vspace{2cm}}}
    \caption{Pipeline di calcolo del sentiment via anchor-based
    cosine similarity.}
...
Embedding semantici bilingui & Il modello all-MiniLM-L6-v2 opera su testi italiani e inglesi tramite anchor vector in entrambe le lingue. \\
Degradazione graceful & Fallback euristico a keyword in caso di indisponibilità del modello di embedding. \\
