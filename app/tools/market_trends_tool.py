...
def filter_outliers_iqr(prices: list[float], items: list[dict]) -> tuple[list[float], list[dict]]:
    """
    Filtra outlier usando il metodo IQR (Interquartile Range), standard statistico
    molto più robusto del precedente filtro basato sulla mediana fissa.
    Restituisce prezzi e items filtrati (mantenendo la corrispondenza).
    """
    if len(prices) < 4:
        return prices, items
...
def classify_trend(current: int, previous: int) -> str:
    """
    Classifica la direzione del trend in 5 livelli invece dei precedenti 3,
    per un verdetto molto più preciso e azionabile.
    """
    if previous == 0:
        return "stabile"
...
def extract_seller_breakdown(shopping_results: list) -> list[dict]:
    """
    Estrae una classifica dei venditori ordinata per prezzo crescente,
    utile per il confronto competitivo nella UI.
    """
    sellers = []
...
async def fetch_google_shopping(query: str, api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # IQR outlier filtering (più robusto della mediana fissa)
        ...
        # Price consistency label per il verdetto
        ...
        return {
            ...
            "median_price": round(median_price, 2),       # NUOVO
            "std_dev": round(std_dev, 2),                  # NUOVO
            "price_range": round(max(prices) - min(prices), 2),  # NUOVO
            "price_consistency": price_consistency,        # NUOVO
            "samples": len(prices),
            "top_results": valid_items[:3],
            "seller_breakdown": extract_seller_breakdown(valid_items),  # NUOVO
        }
...
async def fetch_google_trends(query: str, api_key: str) -> Dict[str, Any]:
    ...
    # Pulizia della query per i trend (più generica del nome prodotto esatto)
    trend_query = query
    ...
    try:
        ...
        # 5 livelli di trend (era solo 3: crescita/calo/stabile)
...
async def fetch_price_history(query: str, api_key: str) -> Dict[str, Any]:
    """
    NUOVA FUNZIONE — Analizza l'interesse di ricerca per 3 finestre temporali
    (24h, 7gg, 3 mesi) per stimare l'andamento recente della domanda.
    Utile per capire se il picco di interesse è appena iniziato o sta finendo.
    """
    from serpapi import GoogleSearch
...
def build_verdetto(shopping_data: Dict, trends_data: Dict) -> str:
    ...
    # Base verdict dal posizionamento del prezzo medio nel range
    spread = max_p - min_p
...
async def execute_market_trends_tool(action_input: Dict[str, Any], context: Any = None) -> Dict[str, Any]:
    ...
    Ora include: filtro IQR, deviazione standard, seller breakdown, 5 livelli di trend,
    storico multi-periodo e verdetto arricchito.
    """
    ...
    if not SERP_API_KEY:
        ...
        return {
            "status": "error",
            "error": "SERP_API_KEY mancante. Configurala nel file .env per usare questo tool."
        }

    # Tutte e 3 le chiamate in parallelo
    shopping_task = fetch_google_shopping(query, SERP_API_KEY)
    trends_task = fetch_google_trends(query, SERP_API_KEY)
    history_task = fetch_price_history(query, SERP_API_KEY)

    shopping_data, trends_data, history_data = await asyncio.gather(
        shopping_task, trends_task, history_task
    )
    ...
    return {
        ...
        "history_data": history_data,   # NUOVO
        "verdetto": verdetto,
    }